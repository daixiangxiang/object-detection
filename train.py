...
#       对数据集进行训练
#-------------------------------------#
#训练moblienet yolov4和训练普通yolov4使用的参数总体上一样，主要差别参数backbone
# (有三个特征提取网络可供选择) ,model path
...
def fit_one_epoch(net, yolo_losses, epoch, epoch_size, epoch_size_val, gen, genval, Epoch, cuda,writer
    ...
    print('一次epoch时间是：'+str(end-begin)+'s')
...
if __name__ == "__main__":
    ...
    #   权值文件请看README，百度网盘下载
    #------------------------------------------------------#
    model_path = "model_data/yolov4_mobilenet_v1_voc.pth"#预训练好的权重文件，需要和主干特征提取网络对应
    # 加快模型训练的效率
