...
def generate_anchor_base(base_size=16, ratios=[0.5, 1, 2],#获得基础的鲜艳框
                         anchor_scales=[8, 16, 32]):

    anchor_base = np.zeros((len(ratios) * len(anchor_scales), 4), dtype=np.float32)
    for i in range(len(ratios)):#循环长度3
...
def _enumerate_shifted_anchor(anchor_base, feat_stride, height, width):
    # 计算网格中心点，feat_stride=16(主干特征提取网络featuremap是输入图片压缩四次)
    #width,height是获得的共享特征层的宽和高
    ...
    return anchor #38*38，每一个特征点上的预测结果对应每一个网格
