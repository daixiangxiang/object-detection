...
def loc2bbox(src_bbox, loc):#src_box鲜艳框，loc建议框网络的预测结果
    if src_bbox.size()[0] == 0:
        return torch.zeros((0, 4), dtype=loc.dtype)

    src_width = torch.unsqueeze(src_bbox[:, 2] - src_bbox[:, 0], -1)#鲜艳框宽高
    src_height = torch.unsqueeze(src_bbox[:, 3] - src_bbox[:, 1], -1)
    src_ctr_x = torch.unsqueeze(src_bbox[:, 0], -1) + 0.5 * src_width#鲜艳框中心
...
class DecodeBox():
    ...
    def forward(self, roi_cls_locs, roi_scores, rois, height, width, nms_iou, score_thresh):
        roi_cls_loc = (roi_cls_locs * self.std + self.mean)#标准化std:0.1,0.1,0.2,0.2.mean0000
        roi_cls_loc = roi_cls_loc.view([-1, self.num_classes, 4])#建议框数量，每一个类，每一个类调整参数
        ...
        output = []
        for l in range(1, self.num_classes):#序号为0，代表框是背景的概览，没有必要考虑
